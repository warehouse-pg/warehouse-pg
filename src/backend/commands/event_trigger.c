			...
			case SCT_AlterTSConfig:
				{
					 ...
					 * catalog lookup and error out for a vanished object, so
					 * skip such rows instead, the same way the IF-NOT-EXISTS/
					 * invalid-OID case above is skipped: there is nothing
					 * coherent left to report about it.
					 *
					 * This only guards the parent relation/object itself, not a
					 * specific column (objectSubId): dropping a user column
					 * doesn't delete its pg_attribute row (RemoveAttributeById()
					 * only does that for system columns), so getObjectIdentity()
					 * already handles that case fine on its own, and there's no
					 * concrete case that needs guarding here.
					 *
