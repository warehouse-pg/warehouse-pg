...
ParallelWorkerMain(Datum main_arg)
{
	 ...
	 * check_gp_is_writer().  Nor can a transaction abort undo it: both are
	 * PGC_BACKEND settings restored with their connection-time source, so
	 * set_config_option() never stacks a rollback entry for them.
	 */
