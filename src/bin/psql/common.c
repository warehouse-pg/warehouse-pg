 ...
 */
static bool
ProcessResult(PGresult **results, int *num_copy_from_stdin)
{
	bool		success = true;
	...
	for (;;)
	{
		...
		if (is_copy)
		{
			...
			else
			{
				...
				{
					/*
					 * We didn't send a COPY FROM STDIN command.  The
					 * server is broken or possibly malicious.  Report and
					 * quit.
					 */
					psql_error("unexpected COPY_IN result, aborting connection\n");
					exit(EXIT_BADCONN);
				}
				(*num_copy_from_stdin)--;
...
SendQuery(const char *query, int num_copy_from_stdin)
{
	...
	if (pset.fetch_count <= 0 || !is_select_command(query))
	{
		...
		/*
		 * If the caller didn't count the number of COPY FROM STDIN
		 * command(s) in the query string, we must do so now.
		 */
		if (num_copy_from_stdin < 0)
		{
			PsqlScanState scan_state;
			PQExpBuffer local_query_buf;
			promptStatus_t prompt_tmp;

			scan_state = psql_scan_create();
			psql_scan_setup(scan_state, query, strlen(query));
			local_query_buf = createPQExpBuffer();

			(void) psql_scan(scan_state, local_query_buf, &prompt_tmp);

			num_copy_from_stdin = psql_scan_count_copy_from_stdin(scan_state);

			destroyPQExpBuffer(local_query_buf);
		...
		ResetCancelConn();
		OK = ProcessResult(&results, &num_copy_from_stdin);
	...
	else
	{
		...
		num_copy_from_stdin = 0;	/* we didn't issue COPY FROM STDIN */
	}
...
sendcopy_cleanup:
	 ...
	 * PGRES_COPY_IN (presumably because the COPY failed server-side), and
	 * the data is supposed to come from the current command source, we must
	 * eat up the data in order to stay in sync with the command file's
	 * contents.  Repeat for the number of unfulfilled COPY commands.
	 */
	...
	/* perform cleanup that should occur even if we never issued the query */
