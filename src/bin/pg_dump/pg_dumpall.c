...
dropDBs(PGconn *conn)
{
	...
	if (PQntuples(res) > 0)
		fprintf(OPF, "--\n-- Drop databases (except postgres, whadmin, gpadmin, and template1)\n--\n\n");
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		...
		/*
		 * Skip "postgres", "whadmin", and "gpadmin"; dumpDatabases() will
		 * deal with them specially.  Also, be sure to skip "template0", even
		 * if for some reason it's not marked !datallowconn.
		 */
		if (strcmp(dbname, "template1") != 0 &&
			strcmp(dbname, "template0") != 0 &&
			strcmp(dbname, "postgres") != 0 &&
			strcmp(dbname, "whadmin") != 0 &&
			strcmp(dbname, "gpadmin") != 0)
		{
...
dumpDatabases(PGconn *conn)
{
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		...
		/*
		 * We assume that "template1", "postgres", "whadmin", and "gpadmin"
		 * already exist in the target installation.  dropDBs() won't have
		 * removed them, for fear of removing the DB the restore script is
		 * initially connected to. If --clean was specified, tell pg_dump to
		 * drop and recreate them; otherwise we'll merely restore their
		 * contents.  Other databases should simply be created.
		 */
		if (strcmp(dbname, "template1") == 0 || strcmp(dbname, "postgres") == 0 ||
			strcmp(dbname, "whadmin") == 0 || strcmp(dbname, "gpadmin") == 0)
		{
			if (output_clean)
