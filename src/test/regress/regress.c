...
test_text_to_wchars(PG_FUNCTION_ARGS)
{
	...
	/*
	 * WarehousePG has no construct_array_builtin(); spell out int4's
	 * elmlen/elmbyval/elmalign instead.
	 */
	PG_RETURN_ARRAYTYPE_P(construct_array(datums, wlen, INT4OID,
										  sizeof(int32), true, 'i'));
}
...
test_wchars_to_text(PG_FUNCTION_ARGS)
{
	...
	/* likewise, no deconstruct_array_builtin() here */
