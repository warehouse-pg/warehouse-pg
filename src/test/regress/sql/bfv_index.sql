...
-- is unchanged and the assertion is on the plan shape. The default-collation
-- index is dropped first so the POSIX one is the only candidate: an unfixed
-- ORCA picks it, a fixed ORCA has no scan alternative left and falls back.
drop index bfv_index_collate_name_idx;
create index bfv_index_collate_name_posix_idx on bfv_index_collate_name_tbl (nm collate "POSIX");
vacuum analyze bfv_index_collate_name_tbl;
explain (costs off) select count(*) from bfv_index_collate_name_tbl where nm >= 'n99';
select count(*) from bfv_index_collate_name_tbl where nm >= 'n99';
-- a "name[]" column's index stays visible: name[] inherits name's built-in
-- C collation (indcollation is C), and gpdb::TypeCollation() now returns C
-- for name[] as it already did for name, so the two agree. Previously it
-- returned the database default for name[], which would have made this
-- index look mismatched and hidden it from ORCA for no reason
