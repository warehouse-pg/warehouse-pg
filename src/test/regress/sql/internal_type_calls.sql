...
-- optimizer_force_multistage_agg only affects ORCA; force it on so the
-- combine functions are exercised even when ORCA would otherwise pick a
-- single-stage plan. This GUC is a no-op under the legacy planner
-- (optimizer=off), but the legacy planner already chooses a two-stage
-- (partial + combine) plan for aggregates over a distributed table like
-- this one when it's cheaper, via gp_enable_multiphase_agg (on by
-- default), which is the case here.
