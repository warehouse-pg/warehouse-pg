...
create table extstats_dim (d_sk int, d_filter int, d_a int, d_b int, d_c int) distributed by (d_sk);
create table extstats_fact (f_sk int, f_val int) distributed by (f_sk);
...
-- Functional-dependency extended statistics: a conjunction of equality
-- filters on the covered columns is estimated as
...
select g, g % 100,
       case when g % 100 < 50 then (g % 100) % 10
            else ((g / 100) * 37 + g % 100) % 100 end
from generate_series(1, 10000) g;
create statistics dep_t_ab (dependencies) on a, b from dep_t;
analyze dep_t;
-- estimated and actual row count of the top plan node
create function dep_estimated_rows(text) returns table (estimated int, actual int)
language plpgsql as
$$
declare
    ln text;
...
set optimizer = on;
-- P(a = 1) = 0.01
select * from dep_estimated_rows('select * from dep_t where a = 1');
-- P(b = 1) = 0.055: 10000 * 0.01 * (0.5 + 0.5 * 0.055) = 53, not 100
select * from dep_estimated_rows('select * from dep_t where a = 1 and b = 1');
-- a range filter on the determining column does not match the dependency;
-- both filters take the regular per-column path
select * from dep_estimated_rows('select * from dep_t where a < 5 and b = 1');
-- IS NULL is a point predicate with a null constant in ORCA's stats
-- predicates; it does not match the dependency either and takes the
-- per-column path
select * from dep_estimated_rows('select * from dep_t where a = 1 and b is null');
