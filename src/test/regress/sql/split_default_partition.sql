...
--  1) the new (non-default) partition inherits storage from the root
--     table, not from the default partition being split (naming is
--     intentionally left as the legacy "<root>_1_prt_<name>" pattern --
--     see the comment in AtExecGPSplitPartition() for why)
--  2) regular (non-default) SPLIT PARTITION is unaffected by (1): it
--     keeps inheriting storage from the partition being split
--  3) SPLIT DEFAULT PARTITION on a default partition with explicit
--     storage is safely observable via a ddl_command_end event trigger
--
-- This whole file runs on its own dedicated schedule line (not
-- alongside other tests) because part 3 installs a database-wide
-- event trigger, which would otherwise also fire for concurrently
-- running tests' DDL.
--

-- 1: storage inheritance for the new (non-default) half
...
-- the new partition keeps the legacy "<root>_1_prt_<name>" name
SELECT relname FROM pg_class WHERE relname = 'split_default_test_1_prt_split_default_new';
...
SELECT c.oid::regclass, amname, reloptions
  ...
  WHERE c.oid IN (SELECT oid FROM pg_class
                  WHERE relname IN ('split_default_test_1_prt_split_default_new',
                                     'split_default_test_dflt'))
  ORDER BY 1::text;

DROP TABLE split_default_test;

-- 2: regular (non-default) SPLIT PARTITION must be unaffected
...
-- 3: SPLIT DEFAULT PARTITION on a default partition with its own explicit
