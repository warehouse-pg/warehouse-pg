...
-- Each statement above runs in its own implicit transaction, so a
-- crashing SPLIT only rolls back that one statement -- the later
-- CREATEs, this SELECT, and the DROPs below all still run regardless.
-- The bug is caught two ways instead: the crashing SPLIT's own
-- "ERROR: cache lookup failed ..." would show up right after it in
-- this file's expected output, and its row(s) would be missing here
-- (a shorter row list than expected) since the INSERT that would have
-- added them never happened.
