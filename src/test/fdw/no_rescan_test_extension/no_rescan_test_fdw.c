...
{
	Cost		startup_cost = 1;
	Cost		total_cost = startup_cost + (baserel->rows * 0.001);
	ForeignPath *path;
	...
	path = create_foreignscan_path(root, baserel,
									   ...
									   NIL);	/* no fdw_private */	if (path != NULL)
	{
...
noRescanIterateForeignScan(ForeignScanState *node)
{
	...
	if (exec_state->current_row < exec_state->max_rows)
	{
		 ...
		 *
		 * Fill values/isnull arrays using slot_get_values/slot_get_isnull
		 */
		values[0] = Int32GetDatum(row_id);
		isnull[0] = false;

		values[1] = CStringGetTextDatum(psprintf("row_%d", row_id));
		isnull[1] = false;
